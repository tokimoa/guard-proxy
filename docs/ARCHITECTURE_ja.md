...
Guard Proxyは、npm / PyPI / RubyGems / Goモジュール / Cargoクレートのサプライチェーン攻撃から開発者を保護するセキュリティプロキシです。
開発者の`npm install` / `pip install` / `gem install` / `go get` / `cargo install`を透過的にインターセプトし、パッケージの安全性を検証してからダウンロードを許可します。
...
```
開発者 (npm install / pip install / gem install / cargo install)
    │
    ▼
┌──────────────────────────────────────────────────────────┐
│                     Guard Proxy                           │
...
└──────────────────────────────────────────────────────────┘
    │
    ▼
上流レジストリ (registry.npmjs.org / pypi.org / rubygems.org / proxy.golang.org / crates.io)
```
...
| `go.py` | GOPROXYプロトコルの実装。モジュールzipのインターセプトとスキャン |
| `cargo.py` | Cargo registry APIプロトコルの実装。crateダウンロードのインターセプト |
...
```
1. GET /api/v1/crates/<crate>        → crateメタデータ取得
2. GET /api/v1/crates/<crate>/<ver>/download → crateダウンロード（ここでスキャン）
```

#### Single-Port Routing（単一ポートルーティング）

すべてのレジストリをパスベースのルーティングで単一ポートから提供できます：

```
localhost:8100/npm/...    → npm registryプロキシ
localhost:8100/pypi/...   → PyPI registryプロキシ
localhost:8100/gems/...   → RubyGems registryプロキシ
localhost:8100/go/...     → Goモジュールプロキシ
localhost:8100/cargo/...  → Cargo registryプロキシ
```

チーム環境でのデプロイやファイアウォール設定を簡素化できます。

### 2. Scanner Layer (`app/scanners/`)

スキャナは二層アーキテクチャを採用しています。**Fast-tier**スキャナはすべてのパッケージに対して常時実行され（軽量、決定論的チェック）、**Slow-tier**スキャナは条件付きまたは遅延実行されます（ネットワーク依存、計算コストが高い）。

**Fast-tierスキャナ**（常時実行）:

1. IOC Checker — 既知の悪意あるパッケージ/ドメインの照合
2. Advisory Scanner — セキュリティアドバイザリデータベースの確認
3. Cooldown Gate — 公開日の鮮度チェック
4. Metadata Scanner — 名前類似度によるタイポスクワッティング検出
5. Maintainer Scanner — メンテナーの信頼性と変更検出
6. Static Analysis — installスクリプトへのパターンマッチング
7. Heuristics Scanner — 動作ヒューリスティック分析
8. AST Scanner — 疑わしい構造の抽象構文木解析
9. YARA Scanner — パッケージ内容へのYARAルールマッチング
10. Reachability Scanner — フラグ付きコードパスの到達可能性分析
11. License Scanner — ライセンスコンプライアンス検証
12. Dependency Scanner — 依存関係ツリーのリスク評価

**Slow-tierスキャナ**（条件付き/遅延実行）:

1. Dependency Graph (deps.dev) — deps.dev APIによる外部依存関係グラフ分析
2. LLM Judge — 曖昧なケースに対するマルチプロバイダーLLM判定

すべてのスキャナが共通の`ScannerProtocol`を実装します。
...
上流レジストリAPI（npmjs.org / pypi.org / rubygems.org / proxy.golang.org / crates.io）との通信を担当します。

- パッケージメタデータ取得（公開日、作者、依存関係リスト）
- tarball / whl / .gem / .zip / .crate ファイルのダウンロード
- httpxベースの非同期HTTPクライアント
- 各レジストリ固有のAPIプロトコル対応（npm JSON API / PyPI Simple API / RubyGems Compact Index / GOPROXY / Cargo Registry API）
...
guard-proxy rules list         # 有効なスキャンルール一覧
guard-proxy rules update       # 上流からスキャンルールを更新
guard-proxy rules add <rule>   # カスタムスキャンルールを追加
guard-proxy rules remove <rule> # スキャンルールを削除
```
...
│   ├── core/                         # 設定管理, ロギング, 例外定義
│   ├── proxy/                        # npm/PyPI/RubyGems/Go/Cargoプロキシエンジン
...
ローカル開発環境:
  Guard Proxy:
    ...
    Admin API:       localhost:8100 (/dashboard でUI表示)
...
## 最近の機能追加 (v2.3.0 -- v2.5.0)

| バージョン | 機能 | 説明 |
|---|---|---|
| v2.3.0 | ライセンスコンプライアンス | License Scannerが設定可能なライセンスポリシー（許可/拒否リスト）に基づきパッケージをチェック。違反時は警告またはインストールをブロック |
| v2.4.0 | YARAマーケットプレイス | コミュニティ提供のYARAルールによる悪意あるパターン検出。`guard-proxy rules list/update/add/remove`で管理可能 |
| v2.5.0 | 到達可能性分析 | Reachability Scannerがフラグ付きコードパスを追跡し、疑わしいコードがinstall時に実際に実行されるか判定。誤検知を削減 |
| v2.5.0 | Cargoサポート | Cargo registryプロキシのフルサポート。crateスキャン、タイポスクワッティング検出、build.rs分析を含む |
| v2.5.0 | 二層スキャンアーキテクチャ | Fast-tier（12個の決定論的スキャナ）は常時実行。Slow-tier（Dependency Graph、LLM Judge）はパフォーマンスのため条件付き/遅延実行 |
| v2.5.0 | Single-port routing | すべてのレジストリプロキシを単一ポートのパスベースルーティングでアクセス可能 |
