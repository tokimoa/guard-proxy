...
Guard Proxyは、npm / PyPI / RubyGems / Goモジュールのサプライチェーン攻撃から開発者を保護するセキュリティプロキシです。
開発者の`npm install` / `pip install` / `gem install` / `go get`を透過的にインターセプトし、パッケージの安全性を検証してからダウンロードを許可します。
...
└──────────────────────────────────────────────────────────┘
    │
    ▼
上流レジストリ (registry.npmjs.org / pypi.org / rubygems.org / proxy.golang.org)
```
