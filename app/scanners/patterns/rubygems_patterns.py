...
CREDENTIAL_ACCESS_PATTERNS: list[MaliciousPattern] = [
    ...
    MaliciousPattern(
        name="ssh_key_access",
        pattern=re.compile(r"""(?:File\.read|Dir\.glob|IO\.read).*\.ssh""", re.IGNORECASE),
        severity="critical",
        description="SSH key file access",
    ),
    MaliciousPattern(
        name="aws_credential_access",
        pattern=re.compile(r"""(?:File\.read|Dir\.glob|IO\.read).*\.aws""", re.IGNORECASE),
        severity="critical",
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(
            r"""(?:File\.read|Dir\.glob|IO\.read).*(?:\.gnupg|\.docker|\.kube|\.config/gcloud)""",
            re.IGNORECASE,
        ),
        severity="high",
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(r"Dir\.home|ENV\[.HOME.\].*(?:Dir\.glob|File\.read|Dir\.entries)", re.IGNORECASE),
        severity="high",
...
NETWORK_EXFIL_PATTERNS: list[MaliciousPattern] = [
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(
            r"(?:Net::HTTP|URI\.open|open-uri|HTTParty|Faraday|RestClient).*ENV",
...
OBFUSCATION_PATTERNS: list[MaliciousPattern] = [
    MaliciousPattern(
        name="base64_eval",
        pattern=re.compile(
            r"(?:Base64\.decode64|Base64\.urlsafe_decode64).*eval|eval.*(?:Base64\.decode64|Base64\.urlsafe_decode64)",
            re.IGNORECASE | re.DOTALL,
        ),
        severity="critical",
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(
            r"""(?:system|exec|%x)\s*[\(\{].*(?:curl|wget|nc\b|bash|sh\b|powershell)""",
            re.IGNORECASE | re.DOTALL,
        ),
        severity="high",
...
PERSISTENCE_PATTERNS: list[MaliciousPattern] = [
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(
            r"""File\.(?:write|open|append).*(?:\.bashrc|\.zshrc|\.profile|\.bash_profile)""",
            re.IGNORECASE,
        ),
        severity="high",
...
ADVANCED_THREAT_PATTERNS: list[MaliciousPattern] = [
    MaliciousPattern(
        name="reverse_shell",
        pattern=re.compile(
            r"TCPSocket\.(?:new|open).*(?:exec|system|spawn|\$stdin\.reopen)",
            re.IGNORECASE | re.DOTALL,
        ),
        severity="critical",
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(r"YAML\.load\s*\((?!.*safe)", re.IGNORECASE),
        severity="high",
        description="Unsafe YAML load (potential RCE)",
    ),
    MaliciousPattern(
        name="erb_eval",
        pattern=re.compile(r"ERB\.new\s*\(.*\)\.result", re.IGNORECASE | re.DOTALL),
        severity="high",
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(r"(?:File\.write|File\.open|IO\.write).*(?:/usr/local/bin|/usr/bin|/bin/)", re.IGNORECASE),
        severity="critical",
        description="System binary path write (PATH hijack)",
    ),
    MaliciousPattern(
        name="git_hook_write",
        pattern=re.compile(r"(?:File\.write|File\.open|IO\.write).*\.git/hooks", re.IGNORECASE),
        severity="critical",
    ...
    MaliciousPattern(
        ...
        pattern=re.compile(r"""(?:File\.read|IO\.read).*\.(?:pem|key|p12|pfx)""", re.IGNORECASE),
        severity="high",
        description="Private key/certificate file access",
    ),
    MaliciousPattern(
        name="env_reflection",
        pattern=re.compile(r"ENV\[.*ENV\[|ENV\.fetch\(.*\+", re.IGNORECASE),
        severity="high",
